...
#                 # 可变形卷积增强几何适应性
#         self.offset_conv = nn.Conv2d(input_dim, 18, 3, padding=1)
#         self.deform_conv = DeformConv2d(input_dim, input_dim, 3, padding=1)
        
#         # 通道注意力机制
#         self.channel_att = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(input_dim*2, input_dim//8, 1),
#             nn.ReLU(),
#             nn.Conv2d(input_dim//8, input_dim*2, 1),
#             nn.Sigmoid()
#         )
...
#                 # 可变形卷积调整水平特征
#         offset = self.offset_conv(hori_feats)
#         hori_feats = self.deform_conv(hori_feats, offset)
#         # 通道注意力融合
#         combined = torch.cat([rot_feats, hori_feats], dim=1)
#         att_map = self.channel_att(combined)
#         rot_att, hori_att = torch.split(att_map, rot_feats.size(1), dim=1)
#         rot_feats = rot_feats * rot_att
#         hori_feats = hori_feats * hori_att
        
#         # 动态权重融合
#         condition_weight = self.condition_net(area_ratio.view(-1, 1))
#         # 由于condition_weight的shape是[1026, 256]，我们需要进行reshape和广播
#         condition_weight = condition_weight.view(-1, 256, 1, 1)  # 将shape调整为[1024, 256, 1, 1]

#         # 确保它可以广播到 [1024, 256, 7, 7]
#         condition_weight = condition_weight.expand_as(rot_feats) 
#         fused_feats = rot_feats + condition_weight * hori_feats
